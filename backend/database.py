...
class Database:
    ...
    async def delete_device(self, mac_address: str) -> bool:
        """Deletes a device and associated records from the database."""
        mac = mac_address.lower().strip()
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("DELETE FROM devices WHERE LOWER(mac_address) = ?", (mac,))
            await db.execute("DELETE FROM connections WHERE LOWER(device_mac) = ?", (mac,))
            await db.execute("DELETE FROM device_scores WHERE LOWER(device_mac) = ?", (mac,))
            await db.commit()
